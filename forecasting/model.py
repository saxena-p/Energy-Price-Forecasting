...
def return_lstm_objective(series_train, X_lag_train, series_valid, X_lag_valid, num_lag_features):
    """
        Returns the LSTM objective function for hyperparameter optimization using Optuna.
        Args:
        the input data.

        Returns:
        lstm_objective (function): The function to be used by Optuna for hyperparameter optimization.
    """
    

    def lstm_objective(trial):
        """Objective function for hyperparameter optimization of the LSTM model using Optuna

        Args:
        trial (Optuna Trial) - trial object to use for hyperparameter optimization

        Returns:
        loss (float) - loss value to minimize
        """
        
        # Define hyperparameters to optimize
        learning_rate = trial.suggest_loguniform('learning_rate', 1e-8, 1e-3)
        momentum = trial.suggest_float('momentum', 0.0, 0.9)
        window_size = trial.suggest_int('window_size', 16, 64, step=8)
        batch_size = trial.suggest_categorical('batch_size', [16, 32, 64])
        shuffle_buffer_size = trial.suggest_int('shuffle_buffer_size', 100, 1000, step=100)
        ...
        # Build the model
        model = get_LSTM_model( window_size = window_size, 
        ...
        # Get the dataset windows with the suggested hyperparameters
        train_set = windowed_dataset_multi_input(
            series=series_train,
        ...
        valid_set = windowed_dataset_multi_input(
            series=series_valid,
        ...
        # Train the model
        history = model.fit(train_set, epochs=10, verbose=0)
        
        # Return the last loss value as the objective to minimize
